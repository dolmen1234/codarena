...
public class Referee extends MultiReferee {
    ...
    public static final String NAME = "GhostInTheCell";
    ...
    final class Factory extends Entity {
        ...
        public Factory(int id, Player owner, int x, int y, int unitCount, int productionRate) {
            super(id, EntityType.FACTORY);
            this.owner = owner;
    ...
    abstract class MovingEntity extends Entity {
        ...
        public MovingEntity(int id, EntityType type, Factory source, Factory destination) {
            super(id, type);
    ...
    final class Bomb extends MovingEntity {
        public Bomb(int id, Factory source, Factory destination) {
            super(id, EntityType.BOMB, source, destination);
        }
    ...
    final class Troop extends MovingEntity {
        private int unitCount;

        public Troop(int id, Factory source, Factory destination, int unitCount) {
            super(id, EntityType.TROOP, source, destination);
            this.unitCount = unitCount;
        }
    ...
    @Override
    public void initReferee(int playerCount, Properties prop) throws InvalidFormatException {
        uniqueEntityId = 0;
    ...
    private void generateFactories() {
        ...
        this.factories[i++] = new Factory(uniqueEntityId++, null, WIDTH / 2, HEIGHT / 2, 0, 0);
        ...
        while (i < factoryCount - 1) {
            ...
            if (valid) {
                ...
                if (i == 1) {
                    ...
                    this.factories[i++] = new Factory(uniqueEntityId++, players[0], x, y, unitCount, productionRate);
                    this.factories[i++] = new Factory(uniqueEntityId++, players[1], WIDTH - x, HEIGHT - y, unitCount, productionRate);
                } else {
                    int unitCount = random.nextInt(5 * productionRate + 1);
                    this.factories[i++] = new Factory(uniqueEntityId++, null, x, y, unitCount, productionRate);
                    this.factories[i++] = new Factory(uniqueEntityId++, null, WIDTH - x, HEIGHT - y, unitCount, productionRate);
                }
            }
        }
    ...
    public void updateGame(int round) throws GameOverException {
        ...
        for (Player player : players) {
            ...
            for (BombAction bombAction : player.lastBombActions) {
                Bomb bomb = new Bomb(uniqueEntityId++, bombAction.src, bombAction.dst);
            ...
            for (MoveAction moveAction : player.lastMoveActions) {
                ...
                Troop troop = new Troop(uniqueEntityId++, moveAction.src, moveAction.dst, unitsToMove);
    ...
    private String translate(String action, int playerId, int actionId) {
        return "";
    }

    private void addToolTip(int playerId, String message) {
        
    }

    private String translate(String message, int id, int id2, int id3) {
        return "";
    }
    ...
    public int getMaxRoundCount(int playerCount) {
        // 200, change to go faster
        return 100;
    }

    @Override
    public int getMillisTimeForRound(int round) {
        return round == 0 ? 1000 : 50;
    }
